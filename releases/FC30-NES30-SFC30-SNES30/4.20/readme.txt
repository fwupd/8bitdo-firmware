Update log:
-----------------------------------------------------------------------------------------------------
2019-05-18  N30+F30_Firmware_V4.20

1. Fixed input lag issue with playing SUPER SMASH BROS.
2. Fixed the pop-up window error when using amiibo.
-------------------------------------------------------------------------------
2017-09-07 N30+F30_Firmware_V4.20

Fixed input lag problem when used with other controllers.
-----------------------------------------------------------------------------------------------------
2017-08-24 N30+F30_Firmware_V4.20

1.修复 Switch mode 混用延迟问题


-----------------------------------------------------------------------------------------------------


2017-05-18 N30+F30_Firmware_V4.00
Firmware 4.0 offers a significant update to all 8Bitdo controllers newly added modes:
1. macOS
	Hold START + A for 1 second to power on the controller.
	Best suited for games on Steam.

2. Nintendo Switch
	Hold START + Y for 1 second
	DOWN+SELECT =  Switch HOME button
-----------------------------------------------------------------------------------------------------
2017-05-18 N30+F30_Firmware_V4.00

1. 新增 Mac 模式，启动方式为 START+A，适用于 Steam 客户端
2. 新增 Switch 模式，启动方式为 START+Y

Note:使用 Switch 模式时，DOWN+SELECT 键为 Swtich HOME 键
